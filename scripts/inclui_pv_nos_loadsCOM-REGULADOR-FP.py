...
OUTPUT_PV = f"PVSystems_{int(PENETRACAO*100)}.dss"
OUTPUT_FP = f"InvControls_{int(PENETRACAO*100)}.dss"
...
FP_ALVO = 0.98   # fator de potência desejado
...
def gera_dss_pv(distribuicao, output_file):
    linhas = []
    ...
    print(f"Arquivo PV DSS gerado: {output_file}")


# ==============================
# 7. GERAR REGULADORES FP
# ==============================

def gera_dss_fp(distribuicao, output_file):
    linhas = []

    for pv in distribuicao:

        linha = (
            f"New InvControl.IC_{pv['nome']} "
            f"DERList=PVSystem.{pv['nome']} "
            f"mode=CONSTANTPF "   # 🔥 CORREÇÃO AQUI
            f"pf={FP_ALVO} "
            f"enabled=yes"
        )
    ...
    print(f"Arquivo InvControl DSS gerado: {output_file}")
...
def main():
    ...
    gera_dss_pv(distribuicao, OUTPUT_PV)
    gera_dss_fp(distribuicao, OUTPUT_FP)
