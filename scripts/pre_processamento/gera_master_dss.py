...
def gerarMaster (): 
    ...
    elementos = listar_arquivos(cf.DSS_DIR)
    loadshapes = listar_arquivos(cf.LS_DIR, filtrar_nome="loadshape")
    loads = listar_arquivos(cf.LOADS_DIR)
    ...
    # Salva arquivo
    with open(cf.DSS_DIR / "Master.dss", "w") as f:
        f.writelines(novo_conteudo)
    
    print(f"Arquivo gerado com sucesso: {cf.DSS_DIR / "Master.dss"}")