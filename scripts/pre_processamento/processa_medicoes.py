...
def processa_medicoes ():
  
    # Planilha com as medições
    planilha = cf.WANG_DIR / "Calculated Nodal P&Q.xlsx"
    ...
    for feeder in feeders:
        ...
        relatorio = []
        relatorio.append(f"\t\tVerificando dataframe do {feeder}")
    ...
    with open(cf.LOGS_DIR / "log_processamento_Nodal_PeQ.txt", "w") as f:
    ...
    plt.legend()
    plt.grid()
    plt.savefig(cf.FIG_NPQ_DIR / "Subestacao_P_Q.png")
    #plt.show()
    plt.close()
    ...
    plt.grid()
    plt.savefig(cf.FIG_NPQ_DIR /  "Subestacao_FP.png")
    #plt.show()
    plt.close()
    ...
    for feeder in feeders:
        ...
        plt.legend()
        plt.grid()
        plt.savefig(cf.FIG_NPQ_DIR / f"{feeder}_P_Q.png")
        plt.show()
        plt.close()
        ...
        plt.grid()
        plt.savefig(cf.FIG_NPQ_DIR / f"{feeder}_FP.png")
        plt.show()
        plt.close()
    ...
    for dia in dias:
        for feeder in feeders:
            ...
            plt.grid()
            plt.savefig(cf.FIG_NPQ_DIR / f"{feeder}_P_Q_dia_{dia}.png")
            plt.show()
            plt.close()
            ...
            plt.grid()
            plt.savefig(cf.FIG_NPQ_DIR / f"{feeder}_FP_dia_{dia}.png")
            plt.show()
            plt.close()
        ...
        plt.legend()
        plt.grid()
        plt.savefig(cf.FIG_NPQ_DIR / f"Subestacao_P_Q_dia_{dia}.png")
        plt.show()
        plt.close()
        ...
        plt.grid()
        plt.savefig(cf.FIG_NPQ_DIR / f"Subestacao_FP_dia_{dia}.png")
        plt.show()
        plt.close()
    ...
    relatorio_geral.extend(f"\nDados processados salvos em: {cf.PROCESSADOS_DIR}/")
    relatorio_geral.append("") 
    # Salva os dataframes gerados
    with pd.ExcelWriter(cf.PROCESSADOS_DIR / "Nodal_P&Q_processado.xlsx") as writer:
    ...
    print(f"\nDados processados salvos em: {cf.PROCESSADOS_DIR}/")
    print(f"\nFiguras salvas em: {cf.FIG_NPQ_DIR}")
